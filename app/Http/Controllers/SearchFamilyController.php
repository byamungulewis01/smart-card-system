...
class SearchFamilyController extends Controller
{
    ...
    public function searchPerson($cardnumber)
    {
        ...
        $admission = Admission::where('card_id', $person->id)->latest()->first();
        if ($person) {
            return response()->json(['success' => true, 'person' => new SearchResource($person), 'admission' => new AdmissionResource($admission)]);
        } else {
